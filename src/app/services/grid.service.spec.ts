...
import {ColDef, ValueFormatterParams, ValueGetterParams, ValueParserParams} from 'ag-grid-community';
...
describe('GridService', () => {
  ...
  it('number columns should always be a number', () => {
    ...
    cols.forEach((col) => {
      if (col.field && col.field !== 'name') {
        const valueParser = col.valueParser as  ((params: ValueParserParams) => any);
  ...
  it('placeholder should only return when no value is present', () => {
    ...
    const valueFormatter = nameCol.valueFormatter as ((params: ValueFormatterParams) => string);
  ...
  it('dollarAmountDisplay should return the correctly truncated values', () => {
    ...
    cols.forEach((col) => {
      if (!col.field && col.headerName !== '') {
        ...
        const valueFormatter = col.valueFormatter as ((params: ValueFormatterParams) => string);
